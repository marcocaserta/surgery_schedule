...
def check_instance_feasibility(instance, alpha_choices, epsilon, max_ot_room, max_ot_doc):
    """
    ...
    for surgery in surgeries:
        ...
        min_buffered_durations.append({
            'id': surgery['id'],
            'min_buffered': min_dur
        })
    ...
    room_capacity_deficit = total_min_buffered - total_max_room_capacity if not room_capacity_feasible else 0
    ...
    doctor_capacity_deficit = total_min_buffered - total_max_doctor_capacity if not doctor_capacity_feasible else 0
    ...
    capacity_feasible = room_capacity_feasible and doctor_capacity_feasible and individual_surgery_feasible
    ...
    return {
        ...
        'capacity_utilization_room': 100 * total_min_buffered / total_regular_room_capacity,
        'capacity_utilization_doctor': 100 * total_min_buffered / total_regular_doctor_capacity,
...
def print_feasibility_report(result):
    ...
    if result['room_capacity_feasible']:
        print(f"    ✅ SUFFICIENT (slack: {result['total_max_room_capacity'] - result['total_min_buffered']:.1f} min)")
    else:
        print(f"    ❌ INSUFFICIENT (deficit: {result['room_capacity_deficit']:.1f} min)")
    ...
    if result['doctor_capacity_feasible']:
        print(f"    ✅ SUFFICIENT (slack: {result['total_max_doctor_capacity'] - result['total_min_buffered']:.1f} min)")
    else:
        print(f"    ❌ INSUFFICIENT (deficit: {result['doctor_capacity_deficit']:.1f} min)")
    
    # Individual surgery check
    print(f"\n  INDIVIDUAL SURGERY vs DOCTOR CAPACITY:")
    print(f"    Max doctor capacity per day: {result['max_doctor_capacity_per_day']:.0f} min")
    ...
    if result['individual_surgery_feasible']:
        ...
        for s in result['infeasible_surgeries']:
            print(f"       • {s['id']:15s}: {s['duration']:6.1f} min (exceeds by {s['excess']:5.1f} min)")
    ...
    print(f"Required per day: Σ ln(1-α_j) ≥ {result['reliability_checks'][0]['required_ln']:.6f}")
    ...
    for check in result['reliability_checks']:
        available = "✓" if check['max_alpha_needed'] <= check['max_alpha_available'] else "✗"
        feasible = "✓" if check['feasible'] else "✗"
        print(f"{check['surgeries_per_day']:>15d} {check['max_alpha_needed']:>15.6f} {available:>12s} {feasible:>12s}")
    
    if result['reliability_feasible']:
        print("\n✅ Reliability constraint is SATISFIABLE")
        print(f"   Feasible distributions: {[r['surgeries_per_day'] for r in result['feasible_distributions']]}")
    else:
    ...
    if not result['overall_feasible']:
        ...
        if not result['capacity_feasible']:
            ...
            if not result['room_capacity_feasible']:
                ...
                print(f"      • Need at least {result['room_capacity_deficit']:.0f} more minutes")
            ...
            if not result['doctor_capacity_feasible']:
                ...
                print(f"      • Need at least {result['doctor_capacity_deficit']:.0f} more minutes")
            ...
            if not result['individual_surgery_feasible']:
                ...
                print(f"      • Increase max doctor overtime by at least {max_excess:.0f} min")
                print(f"      • Increase doctor daily capacity (H)")
                print(f"      • Use more aggressive α values to reduce buffered durations")
...
def main():
    ...
    parser.add_argument('--epsilon', type=float, default=0.25,
                       help='Epsilon value (default: 0.25)')
    parser.add_argument('--max-ot-room', type=int, default=120,
                       help='Max room overtime (default: 120)')
    parser.add_argument('--max-ot-doc', type=int, default=60,
                       help='Max doctor overtime (default: 60)')
