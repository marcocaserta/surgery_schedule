...
def generate_instance(num_surgeries=35, num_days=5, num_rooms=5, num_doctors=6, H=960):
    ...
    days = [{"id": day_names[i % 7], "H": H, "name": day_names[i % 7]} for i in range(num_days)]
    ...
    all_combinations = [f"{room['id']}|{doctor['id']}" for room in rooms for doctor in doctors]
    ...
    for i in range(1, small_count + 1):
        ...
        for key in all_combinations:
            ...
            mu_sigma_dict[key] = {
                "mu": base_mu + (doc_num - num_doctors/2) * 2 + (room_num - num_rooms/2) * 1,
    ...
    for i in range(1, medium_count + 1):
        ...
        for key in all_combinations:
            ...
            mu_sigma_dict[key] = {
                "mu": base_mu + (doc_num - num_doctors/2) * 3 + (room_num - num_rooms/2) * 1.5,
    ...
    for i in range(1, large_count + 1):
        ...
        for key in all_combinations:
            ...
            mu_sigma_dict[key] = {
                "mu": base_mu + (doc_num - num_doctors/2) * 4 + (room_num - num_rooms/2) * 2,
...
def generate_35_surgery_instance():
    ...
    dict : Problem instance with surgeries, days, rooms, doctors
    """
    return generate_instance(num_surgeries=35, num_days=5, num_rooms=5, num_doctors=6, H=960)
...
def print_instance_overview(instance):
    ...
    total_demand = sum(s.get('mu', s.get('duration_mean', 0)) for s in instance['surgeries'])
    
    print(f"\n  Capacity Analysis:")
    print(f"    • Total room capacity: {total_capacity:,} minutes")
    print(f"    • Total mean duration: {total_demand:,.0f} minutes")
    print(f"    • Capacity utilization (mean): {100 * total_demand / total_capacity:.1f}%")
