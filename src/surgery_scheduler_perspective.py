...
class SurgeryScheduler:
    ...
    def build_model(self):
        ...
        # Perspective variables: s_persp[j,d,r,k,t] = s[j,d,r] * w[j,d,r,k,t]
        print("Adding perspective variables...")
        model.s_persp = pyo.Var(model.VALID, 
                               bounds=lambda m,j,d,r,k,t: (0.0, day_dict[d]["H"] + self.max_ot_room),
                               domain=pyo.NonNegativeReals)
        
        # Perspective for doctor start times: s_doc_persp[j,d,r,k,t] = s_doc[j,d,k] * w[j,d,r,k,t]
        model.s_doc_persp = pyo.Var(model.VALID,
                                   bounds=lambda m,j,d,r,k,t: (0.0, day_dict[d]["H"] + self.max_ot_doc),
                                   domain=pyo.NonNegativeReals)
        
        print(f"  ✓ Added {len(valid)} perspective variables for room start times")
        print(f"  ✓ Added {len(valid)} perspective variables for doctor start times")
    ...
    def _identify_cliques(self, J, D, R, K, day_dict, doc_dict):
        """
        Identify cliques: sets of surgeries that cannot fit together on same day/room/doctor.
        Returns dict: {(d,r,k): [list of clique sets]}
        """
        cliques = {}
        
        # Compute minimum durations for each surgery
        min_durations = {}
        for j in J:
            min_dur = min([self.durations[j][r][k][t] 
                          for r in R for k in K for t in range(len(self.alpha_choices))
                          if self.durations[j][r][k].get(t) is not None], 
                         default=float('inf'))
            min_durations[j] = min_dur
        
        # For each (d,r,k) combination, find cliques
        for d in D:
            for r in R:
                for k in K:
                    # Get capacity for this combination
                    if k in doc_dict:
                        doctor_capacity = doc_dict[k].get('daily_capacity', {})
                    ...
                    max_capacity = min(day_dict[d]['H'] + self.max_ot_room, cap + self.max_ot_doc)
                    
                    # Find surgeries that can be assigned to this (d,r,k)
                    eligible = []
                    for j in J:
                        if self.durations[j][r][k] and any(self.durations[j][r][k].get(t) is not None 
                                                           for t in range(len(self.alpha_choices))):
                            eligible.append(j)
                    ...
                    # Start with pairs that don't fit
                    clique_list = []
                    for i, j1 in enumerate(eligible):
                        for j2 in eligible[i+1:]:
                            if min_durations[j1] + min_durations[j2] > max_capacity:
                                # These two can't fit together - start a clique
                                clique = {j1, j2}
                                
                                # Try to extend clique
                                for j3 in eligible:
                                    if j3 not in clique:
                                        # Check if j3 conflicts with all members
                                        if all(min_durations[j3] + min_durations[jc] > max_capacity 
                                              for jc in clique):
                                            clique.add(j3)
                                
                                # Only add if clique size >= 2
                                if len(clique) >= 2:
                                    clique_list.append(frozenset(clique))
                    
                    # Remove duplicate cliques
                    if clique_list:
                        cliques[(d,r,k)] = list(set(clique_list))
    ...
    def _add_constraints(self, m, day_dict, doc_dict, M_room, pruned_u_doc):
        ...
        # C1b: Clique inequalities
        print("Identifying cliques...")
        cliques = self._identify_cliques(J, D, R, K, day_dict, doc_dict)
        total_cliques = sum(len(v) for v in cliques.values())
        print(f"  Found {total_cliques} cliques across {len(cliques)} (d,r,k) combinations")
        
        clique_constraints = []
        for (d, r, k), clique_list in cliques.items():
            for clique in clique_list:
                clique_constraints.append((d, r, k, clique))
        
        def clique_rule(m, idx):
            d_val, r_val, k_val, clique = clique_constraints[idx]
            return sum(m.w[j,d_val,r_val,k_val,t] 
                      for j in clique 
                      for t in m.T 
                      if (j,d_val,r_val,k_val,t) in m.VALID) <= 1
        
        if clique_constraints:
            m.clique = pyo.Constraint(range(len(clique_constraints)), rule=clique_rule)
            print(f"  ✓ Added {len(clique_constraints)} clique constraints")
        
        # C1c: McCormick linearization for perspective variables
        print("Adding McCormick linearization constraints...")
        
        # For s_persp[j,d,r,k,t] = s[j,d,r] * w[j,d,r,k,t]
        def persp_lb1_rule(m, j, d, r, k, t):
            return m.s_persp[j,d,r,k,t] >= 0
        m.persp_lb1 = pyo.Constraint(m.VALID, rule=persp_lb1_rule)
        
        def persp_lb2_rule(m, j, d, r, k, t):
            UB = day_dict[d]["H"] + self.max_ot_room
            return m.s_persp[j,d,r,k,t] >= m.s[j,d,r] - UB * (1 - m.w[j,d,r,k,t])
        m.persp_lb2 = pyo.Constraint(m.VALID, rule=persp_lb2_rule)
        
        def persp_ub1_rule(m, j, d, r, k, t):
            UB = day_dict[d]["H"] + self.max_ot_room
            return m.s_persp[j,d,r,k,t] <= UB * m.w[j,d,r,k,t]
        m.persp_ub1 = pyo.Constraint(m.VALID, rule=persp_ub1_rule)
        
        def persp_ub2_rule(m, j, d, r, k, t):
            return m.s_persp[j,d,r,k,t] <= m.s[j,d,r]
        m.persp_ub2 = pyo.Constraint(m.VALID, rule=persp_ub2_rule)
        
        # For s_doc_persp[j,d,r,k,t] = s_doc[j,d,k] * w[j,d,r,k,t]
        def persp_doc_lb1_rule(m, j, d, r, k, t):
            return m.s_doc_persp[j,d,r,k,t] >= 0
        m.persp_doc_lb1 = pyo.Constraint(m.VALID, rule=persp_doc_lb1_rule)
        
        def persp_doc_lb2_rule(m, j, d, r, k, t):
            if k in doc_dict:
                doctor_capacity = doc_dict[k].get('daily_capacity', {})
                cap = doctor_capacity.get(d, day_dict[d]['H']) if isinstance(doctor_capacity, dict) else day_dict[d]['H']
            else:
                cap = day_dict[d]['H']
            UB = cap + self.max_ot_doc
            return m.s_doc_persp[j,d,r,k,t] >= m.s_doc[j,d,k] - UB * (1 - m.w[j,d,r,k,t])
        m.persp_doc_lb2 = pyo.Constraint(m.VALID, rule=persp_doc_lb2_rule)
        
        def persp_doc_ub1_rule(m, j, d, r, k, t):
            if k in doc_dict:
                doctor_capacity = doc_dict[k].get('daily_capacity', {})
                cap = doctor_capacity.get(d, day_dict[d]['H']) if isinstance(doctor_capacity, dict) else day_dict[d]['H']
            else:
                cap = day_dict[d]['H']
            UB = cap + self.max_ot_doc
            return m.s_doc_persp[j,d,r,k,t] <= UB * m.w[j,d,r,k,t]
        m.persp_doc_ub1 = pyo.Constraint(m.VALID, rule=persp_doc_ub1_rule)
        
        def persp_doc_ub2_rule(m, j, d, r, k, t):
            return m.s_doc_persp[j,d,r,k,t] <= m.s_doc[j,d,k]
        m.persp_doc_ub2 = pyo.Constraint(m.VALID, rule=persp_doc_ub2_rule)
        
        print(f"  ✓ Added {8 * len(m.VALID)} McCormick linearization constraints")
        
        # C2: Room capacity with overtime (PERSPECTIVE FORMULATION)
        def room_cap_rule(m, d, r):
            # Sum perspective start times + durations for all surgeries in this room
            return sum(m.s_persp[j,d,r,k,t] + self.durations[j][r][k][t] * m.w[j,d,r,k,t]
                      for (j,d2,r2,k,t) in m.VALID
                      if d2==d and r2==r and self.durations[j][r][k].get(t) is not None) <= day_dict[d]["H"] + m.OT_room[d,r]
        m.room_cap = pyo.Constraint(m.D, m.R, rule=room_cap_rule)
