...
class SurgeryScheduler:
    ...
    def build_model(self):
        ...
        if isinstance(self.epsilon, (int, float)):
            ...
            epsilon_dict = {
                d["id"]: float(self.epsilon.get(d["id"], 0.25))
        ...
        fixed_vars = self._preprocess_and_fix(
            J, D, R, K, T, day_dict, doc_dict, surgery_dict
        )
        ...
        model.log_one_minus_eps = pyo.Param(
            model.D,
        ...
        model.u_room = pyo.Var(
            [(j, i, d, r) for j in J for i in J for d in D for r in R if j < i],
            domain=pyo.Binary,
        )
        ...
        model.OT_doc = pyo.Var(
            model.D, model.K, bounds=(0.0, self.max_ot_doc), domain=pyo.NonNegativeReals
        )

        model.Idle = pyo.Var(model.D, model.R, domain=pyo.NonNegativeReals)

        model.y = pyo.Expression(
            model.J,
            model.D,
            model.K,
            rule=lambda m, j, d, k: sum(
                m.w[j, d, r, k, t]
        ...
        # Big-M
        max_dur = max(
            max(self.durations[j][r][k].values())
            for j in J
            for r in R
            for k in K
        ...
        def obj_rule(m):
            ...
            room_ot_cost = self.ot_cost_room * sum(
                m.OT_room[d, r] for d in D for r in R
            )
    ...
    def _add_constraints(self, m, day_dict, doc_dict, M_room, pruned_u_doc):
        ...
        def assign_rule(m, j):
            return (
                sum(m.w[j, d, r, k, t] for (j2, d, r, k, t) in m.VALID if j2 == j) == 1
            )
        ...
        def room_cap_rule(m, j, d, r):
            ...
            dur_sum = sum(
                self.durations[j][r][k][t] * m.w[j, d, r, k, t]
                for (j2, d2, r2, k, t) in m.VALID
                if j2 == j
                and d2 == d
                and r2 == r
                and self.durations[j][r][k].get(t) is not None
            )
            return m.s[j, d, r] + dur_sum <= day_dict[d]["H"] + m.OT_room[
                d, r
            ] + M_room[d] * (1 - chosen)
        ...
        def doc_cap_rule(m, k, d):
            if k in doc_dict:
                doctor_capacity = doc_dict[k].get("daily_capacity", {})
                cap = (
                    doctor_capacity.get(d, day_dict[d]["H"])
                    if isinstance(doctor_capacity, dict)
                    else day_dict[d]["H"]
                )
            else:
        ...
        def sdoc_upper_rule(m, j, d, k, r):
            return m.s_doc[j, d, k] <= m.s[j, d, r] + M_room[d] * (
                1 - sum(m.w[j, d, r, k, t] for t in m.T if (j, d, r, k, t) in m.VALID)
            )
        ...
        def sdoc_upper_H_rule(m, j, d, k):
            return m.s_doc[j, d, k] <= day_dict[d]["H"] + m.OT_doc[d, k] + M_room[d] * (
                1 - m.y[j, d, k]
            )
        ...
        def s_room_upper_rule(m, j, d, r):
            ...
            return m.s[j, d, r] <= day_dict[d]["H"] + m.OT_room[d, r] + M_room[d] * (
                1 - chosen
            )
        ...
        def room_seq_fwd_rule(m, j, i, d, r):
            ...
            return m.s[i, d, r] >= m.s[j, d, r] + dur_j - M_room[d] * (
                2 - chosen_j - chosen_i
            ) - M_room[d] * (1 - m.u_room[j, i, d, r])

        m.room_seq_fwd = pyo.Constraint(
            [(j, i, d, r) for j in J for i in J for d in D for r in R if j < i],
        ...
        def room_seq_bwd_rule(m, j, i, d, r):
            ...
            dur_i = sum(
                self.durations[i][r][k][t] * m.w[i, d, r, k, t]
                for (i2, d2, r2, k, t) in m.VALID
                if i2 == i
                and d2 == d
                and r2 == r
                and self.durations[i][r][k].get(t) is not None
            )
            ...
            return (
                ...
                >= m.s[i, d, r]
                + dur_i
                - M_room[d] * (2 - chosen_j - chosen_i)
                - M_room[d] * m.u_room[j, i, d, r]
            )

        m.room_seq_bwd = pyo.Constraint(
            [(j, i, d, r) for j in J for i in J for d in D for r in R if j < i],
        ...
        def doctor_seq_fwd_rule(m, j, i, d, k):
            if (j, i, d, k) not in m.u_doc.index_set():
                return pyo.Constraint.Skip
            return m.s_doc[i, d, k] >= m.s_doc[j, d, k] + m.dur_doc[j, d, k] - M_room[
                d
            ] * (2 - m.y[j, d, k] - m.y[i, d, k]) - M_room[d] * (
                1 - m.u_doc[j, i, d, k]
            )
        ...
        def doctor_seq_bwd_rule(m, j, i, d, k):
            ...
            return (
                ...
                >= m.s_doc[i, d, k]
                + m.dur_doc[i, d, k]
                - M_room[d] * (2 - m.y[j, d, k] - m.y[i, d, k])
                - M_room[d] * m.u_doc[j, i, d, k]
            )
        ...
        def reliability_rule(m, d):
            return (
                sum(
                    ln_alpha[t] * m.w[j, d, r, k, t]
                    for (j, d2, r, k, t) in m.VALID
                    if d2 == d
                )
                >= m.log_one_minus_eps[d]
            )
        ...
        m.room_day_aggregate_cut = pyo.Constraint(
            m.D, m.R, rule=room_day_aggregate_cut_rule
        )
        ...
        m.doc_day_aggregate_cut = pyo.Constraint(
            m.D, m.K, rule=doc_day_aggregate_cut_rule
        )

        # --- Sequencing linkage constraints ---

        with_sequencing = False  # does not seem to help
        if with_sequencing:

            def room_seq_link_rule(m, j, i, d, r):
                """Link u_room to assignment decisions."""
                if (j, i, d, r) not in m.u_room.index_set():
                    return pyo.Constraint.Skip
                chosen_j = sum(
                    m.w[j, d, r, k, t]
                ...
                # If either surgery isn't assigned to this room/day, u_room must be 0
                return m.u_room[j, i, d, r] <= 0.5 * (chosen_j + chosen_i)

            m.room_seq_link = pyo.Constraint(
                [(j, i, d, r) for j in J for i in J for d in D for r in R if j < i],
                rule=room_seq_link_rule,
            )

            def room_seq_order_rule(m, j, i, d, r):
                """Ensure at least one ordering if both surgeries share (d,r)."""
                if (j, i, d, r) not in m.u_room.index_set():
                    return pyo.Constraint.Skip
                chosen_j = sum(
                    m.w[j, d, r, k, t]
                ...
                return m.u_room[j, i, d, r] >= chosen_j + chosen_i - 1

            m.room_seq_order = pyo.Constraint(
                [(j, i, d, r) for j in J for i in J for d in D for r in R if j < i],
                rule=room_seq_order_rule,
            )

            # --- Repeat the same idea for doctors ---

            def doc_seq_link_rule(m, j, i, d, k):
                if (j, i, d, k) not in m.u_doc.index_set():
                    return pyo.Constraint.Skip
                chosen_j = sum(
                    m.w[j, d, r, k, t]
                ...
                chosen_i = sum(
                    m.w[i, d, r, k, t]
                    for (i2, d2, r, k2, t) in m.VALID
                    if i2 == i and d2 == d and k2 == k
                )
                return m.u_doc[j, i, d, k] <= 0.5 * (chosen_j + chosen_i)

            m.doc_seq_link = pyo.Constraint(pruned_u_doc, rule=doc_seq_link_rule)

            def doc_seq_order_rule(m, j, i, d, k):
                if (j, i, d, k) not in m.u_doc.index_set():
                    return pyo.Constraint.Skip
                chosen_j = sum(
                    m.w[j, d, r, k, t]
                ...
                return m.u_doc[j, i, d, k] >= chosen_j + chosen_i - 1

            m.doc_seq_order = pyo.Constraint(pruned_u_doc, rule=doc_seq_order_rule)
