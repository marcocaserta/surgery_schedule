...
class SurgeryScheduler:
    ...
    def build_model(self):
        ...
        # Compute surgery-specific Big-M values
        # M[j,d,r] = max time surgery j can end in room r on day d
        M_surgery_room = {}
        for j in J:
            for d in D:
                for r in R:
                    # Maximum possible end time = H + max_ot_room + max duration of surgery j in room r
                    max_dur_j = max([self.durations[j][r][k].get(t, 0) 
                                    for k in K for t in T 
                                    if self.durations[j][r][k].get(t) is not None], 
                                   default=0)
                    M_surgery_room[j,d,r] = day_dict[d]["H"] + self.max_ot_room + max_dur_j
        
        # M for sequencing: max time between two surgeries
        M_seq_room = {}
        for j in J:
            for i in J:
                if j != i:
                    for d in D:
                        for r in R:
                            # Max time = max duration of j + max duration of i
                            max_dur_j = max([self.durations[j][r][k].get(t, 0) 
                                           for k in K for t in T 
                                           if self.durations[j][r][k].get(t) is not None], 
                                          default=0)
                            max_dur_i = max([self.durations[i][r][k].get(t, 0) 
                                           for k in K for t in T 
                                           if self.durations[i][r][k].get(t) is not None], 
                                          default=0)
                            # Use room capacity + max overtime as upper bound
                            M_seq_room[j,i,d,r] = min(day_dict[d]["H"] + self.max_ot_room, 
                                                     max_dur_j + max_dur_i)
        
        # M for doctor sequencing
        M_seq_doc = {}
        for j in J:
            for i in J:
                if j != i:
                    for d in D:
                        for k in K:
                            ...
                            # Max time = doctor capacity + max overtime
                            max_dur_j = max([self.durations[j][r][k].get(t, 0) 
                            ...
                            max_dur_i = max([self.durations[i][r][k].get(t, 0) 
                                           ...
                                           if self.durations[i][r][k].get(t) is not None], 
                                          default=0)
                            M_seq_doc[j,i,d,k] = min(cap + self.max_ot_doc, 
                                                    max_dur_j + max_dur_i)
        
        # Generic M for synchronization constraints (use max across all)
        M_sync = max(day_dict[d]["H"] + self.max_ot_room for d in D)
        
        # Constraints
        self._add_constraints(model, day_dict, doc_dict, M_surgery_room, M_seq_room, M_seq_doc, M_sync, pruned_u_doc)
    ...
    def _add_constraints(self, m, day_dict, doc_dict, M_surgery_room, M_seq_room, M_seq_doc, M_sync, pruned_u_doc):
        ...
        def room_cap_rule(m, j, d, r):
            ...
            return m.s[j,d,r] + dur_sum <= day_dict[d]["H"] + m.OT_room[d,r] + M_surgery_room[j,d,r] * (1 - chosen)
        ...
        def sdoc_upper_rule(m, j, d, k, r):
            return m.s_doc[j,d,k] <= m.s[j,d,r] + M_sync * (1 - sum(m.w[j,d,r,k,t] 
                                                                        for t in m.T 
        ...
        def sdoc_upper_H_rule(m, j, d, k):
            return m.s_doc[j,d,k] <= day_dict[d]["H"] + m.OT_doc[d,k] + M_sync * (1 - m.y[j,d,k])
        ...
        def s_room_upper_rule(m, j, d, r):
            chosen = sum(m.w[j,d,r,k,t] for (j2,d2,r2,k,t) in m.VALID
                        if j2==j and d2==d and r2==r)
            return m.s[j,d,r] <= day_dict[d]["H"] + m.OT_room[d,r] + M_surgery_room[j,d,r] * (1 - chosen)
        ...
        def room_seq_fwd_rule(m, j, i, d, r):
            ...
            M_seq = M_seq_room[j,i,d,r]
            return m.s[i,d,r] >= m.s[j,d,r] + dur_j - M_seq * (2 - chosen_j - chosen_i) - M_seq * (1 - m.u_room[j,i,d,r])
        ...
        def room_seq_bwd_rule(m, j, i, d, r):
            ...
            M_seq = M_seq_room[j,i,d,r]
            return m.s[j,d,r] >= m.s[i,d,r] + dur_i - M_seq * (2 - chosen_j - chosen_i) - M_seq * m.u_room[j,i,d,r]
        ...
        def doctor_seq_fwd_rule(m, j, i, d, k):
            if (j,i,d,k) not in m.u_doc.index_set():
                return pyo.Constraint.Skip
            M_seq = M_seq_doc[j,i,d,k]
            return m.s_doc[i,d,k] >= m.s_doc[j,d,k] + m.dur_doc[j,d,k] - M_seq * (2 - m.y[j,d,k] - m.y[i,d,k]) - M_seq * (1 - m.u_doc[j,i,d,k])
        ...
        def doctor_seq_bwd_rule(m, j, i, d, k):
            if (j,i,d,k) not in m.u_doc.index_set():
                return pyo.Constraint.Skip
            M_seq = M_seq_doc[j,i,d,k]
            return m.s_doc[j,d,k] >= m.s_doc[i,d,k] + m.dur_doc[i,d,k] - M_seq * (2 - m.y[j,d,k] - m.y[i,d,k]) - M_seq * m.u_doc[j,i,d,k]
